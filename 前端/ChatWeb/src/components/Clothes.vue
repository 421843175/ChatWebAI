...
export default {
  ...
  computed: {
    ...
    editorFrameStyle() {
      const size = Math.round(800 * this.editorScale)
      return {
        width: `${size}px`,
        height: `${size}px`
      }
    ...
    editorCanvasStyle() {
      return {
        transform: `scale(${this.editorScale})`,
  ...
  methods: {
    ...
    other(tree){
      // 空数据保护
      if (!tree || !tree.a_front || !tree.a_front[this.gender] || !tree.a_front[this.gender].Hair_Front || !tree.a_front[this.gender].Hair_Behind || !tree.b_decorate || !tree.b_decorate[this.gender] || !tree.c_clothes || !tree.c_clothes[this.gender]) {
        console.warn('形象分类数据未加载完成');
        return;
      }
    ...
    loadBody(tree){
      // 空数据保护
      if (!tree || !tree.d_model || !tree.d_model[this.gender] || !tree.d_model[this.gender].children || !tree.d_model[this.gender].children[0]) {
        console.warn('人物模型数据未加载完成，等待中...');
        return;
      }
    ...
    initData(){
      ...
      this.$http.post(this.$constant.baseURL + "/model/getModel",usertoken).then(response => {
            if(response.data.code==200 && response.data.data){
              ...
              console.log("未找到已保存的人物信息，将使用默认形象")
            }
...
<template>
  <div class="root">
    ...
    <div
        ...
        @click="c"
    >
      <div class="container" :style="editorFrameStyle">
        <div class="container-canvas" :style="editorCanvasStyle">
          <div  class="target"
                :data-id="layer.id"
...
.root {
  position: relative;
  display: grid;
  grid-template-columns: 220px minmax(520px, 1fr) minmax(340px, 42%);
  align-items: stretch;
...
@media (max-width: 1680px) {
  .root {
    grid-template-columns: 180px minmax(460px, 1fr) minmax(300px, 38%);
  }
}

@media (max-width: 1440px) {
  .root {
    grid-template-columns: 160px minmax(420px, 1fr) minmax(280px, 36%);
    gap: 12px;
  }
}
